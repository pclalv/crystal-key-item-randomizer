defmodule SwapGenerator.Reachability.KantoLockedTest do
  use ExUnit.Case, async: true
  alias SwapGenerator.Reachability.KantoLocked
  doctest KantoLocked
  ...
  test "if it's possible to reach kanto because the PASS is not in kanto" do
    non_kanto_items = SwapGenerator.key_item_names() -- SwapGenerator.kanto_items()
  ...
  test "if it's not possible to reach kanto and a required item is in kanto" do
    ...
    swaps =
      ...
      |> Map.put(random_item3, Enum.random(SwapGenerator.required_item_names()))
  ...
  test "if it's not possible to reach kanto and a maybe-required item is required and its prereq is in kanto" do
    ...
    non_kanto_maybe_requireds =
      for {prereq, maybe_required} <- SwapGenerator.maybe_required_pairs(),
    ...
    swaps =
      ...
      |> Map.put(maybe_required, Enum.random(SwapGenerator.required_item_names()))
