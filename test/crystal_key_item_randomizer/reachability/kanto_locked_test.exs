defmodule LockDetector.Reachability.KantoLockedTest do
  ...
  test "if it's not possible to reach kanto and a required item is in kanto" do
    [random_item1, random_item2, random_item3] = LockDetector.kanto_items
    |> Enum.take_random(3)
    swaps = %{}
  ...
  test "if it's not possible to reach kanto and a maybe-required item is required and its prereq is in kanto" do
    [random_item1, random_item2, random_item3] = LockDetector.kanto_items
    |> Enum.take_random(3)
    non_kanto_maybe_requireds = (for {prereq, maybe_required} <- LockDetector.maybe_required_pairs, Enum.member?([:BASEMENT_KEY, :CARD_KEY], maybe_required), do: {prereq, maybe_required}, into: %{})
