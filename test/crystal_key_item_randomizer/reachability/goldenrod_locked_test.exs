defmodule LockDetector.Reachability.GoldenrodLockedTest do
  use ExUnit.Case, async: true
  alias LockDetector.Reachability.GoldenrodLocked
  ...
  test "when PASS is obtainable, and SUPER_ROD is the SQUIRTBOTTLE" do
    swaps = (for item <- LockDetector.pre_goldenrod_items, do: {item, :FOO}, into: %{})
    |> Map.put(Enum.random(LockDetector.pre_goldenrod_items), :PASS)
  ...
  test "when SQUIRTBOTTLE is obtainable" do
    swaps = (for item <- LockDetector.pre_goldenrod_items, do: {item, :FOO}, into: %{})
    |> Map.put(Enum.random(LockDetector.pre_goldenrod_items), :SQUIRTBOTTLE)
    result = Diet.Stepper.new(GoldenrodLocked, nil)
    |> Diet.Stepper.run({:begin, swaps})
  ...
  test "when SQUIRTBOTTLE and PASS are obtainable" do
    [random_item1, random_item2] = LockDetector.pre_goldenrod_items
    |> Enum.take_random(2)
    swaps = (for item <- LockDetector.pre_goldenrod_items, do: {item, :FOO}, into: %{})
